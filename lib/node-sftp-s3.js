...
class SFTPServerStorage extends EventEmitter {
	 ...
	 * @param {string} bucket
	 * @param authFn - function(ctx) - do ctx.resolve if successful, ctx.reject(Array<string> supportedAuthMethods) if not
	 */
	constructor(storage, bucket, authFn) {
    super();
          ...
          session.on('sftp', (accept, reject) => {
              ...
              if(offset + length >= state.size) {
                state.read = true;
				  ...
				  this.storage.getObject(state.fullname, `bytes=${offset}-${offset+length-1}`, function(err, data) {
